...
Health endpoints are available through Quarkus SmallRye Health:
...
GitHub Actions workflow is available at `.github/workflows/ci-cd.yml` and includes:

1. **CI** on pull requests and pushes to `main`: runs `./mvnw -Dtest='*Test,*IT' test` on Java 21.
2. **Coverage gate** on CI: computes JaCoCo line coverage from `target/jacoco-report/jacoco.csv` and fails the build if coverage is below **80%**.
3. **CD packaging** on `main` pushes (and manual trigger): builds the JAR and uploads it as a workflow artifact.

JaCoCo report is generated at `target/jacoco-report/index.html`.